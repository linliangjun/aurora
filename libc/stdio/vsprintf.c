...
static char *string(char *buf, const char *str, uint32_t width, int32_t precision, bool left)
{
    // 计算出实际展示的字符串长度
 ...
 */
static uint32_t skip_atoi(const char **s)
{
    uint32_t i = 0;
    while (isdigit(**s))
        i = i * 10 + *((*s)++) - '0';
 ...
 * @return 返回指向缓冲区末尾的指针
 */
static char *number(char *buf, uint32_t num, uint8_t base, uint32_t width, int32_t precision, format_flags_t flags)
{
    char sign = 0;

    // 如果是有符号数，且 num > INT_MAX，则 sign = '-'，并使 num 取绝对值
    if (flags.sign && num > INT_MAX)
    {
        sign = '-';
        num = ~num + 1;
    }
    ...
    // 计算出需要填充的空格或 0 数量
    uint32_t padding = width <= precision ? 0 : width - precision;
    ...
    uint32_t gap = i < precision ? precision - i : 0;
    memset(buf, '0', gap);
    buf += gap;
    while (i--)
